...
/*
 * Sega Master System sound effects on the TI SN76489 PSG (write-only I/O port
 * 0x7F). The chip has three square-wave tone channels and one noise channel; each
 * takes a 10-bit period and a 4-bit attenuation (0 = loudest, 15 = silent). We
 * drive it directly with port writes — the same approach (and tuning) as
 * src/adam/sound.c, only the I/O port differs (0x7F here vs 0xFF on the Adam).
 *
 * Periods are precomputed (n = 223722 / Hz) to avoid a runtime 32-bit divide;
 * tone frequency is clock / (32 * n). The SMS PSG clock (3.579545 MHz) matches the
 * Adam's, so the constants port over unchanged.
 */
#include <stdint.h>
...
/* Precomputed periods. Bigger n = lower pitch. */
#define N_C5  428u
#define N_E5  339u
#define N_G5  285u
#define N_C6  214u
 ...
 * (midi - music_note_lo). n = 111860 / Hz (the true octave); shared tuning with
 * the Adam/ColecoVision player. */
static const unsigned int hymn_sn[11] = {
    226, 214, 202, 190, 180, 170, 160, 151, 143, 135, 127  /* B4 C5 .. A5 */
};

/* Fine-grained music delay (~10ms per unit) — the sfx hold() is far too coarse
 * for a melody. Matches the Adam's mdelay tuning (same Z80 clock). */
...
#define SMS_MUS_EIGHTH 28u

/* Play one melody note (MIDI number, or MUSIC_REST) for `eighths` eighth-note
 * ticks on tone channel 0, with a short note-off gap so repeated pitches
 * articulate. The melody loop + the skip poll live in main.c. */
void sms_music_note(unsigned char midi, unsigned char eighths)
{
    unsigned int units = (unsigned int)eighths * SMS_MUS_EIGHTH;
