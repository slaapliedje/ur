...
# perl = Game Boy CGB header patch; bash + zip = `make release` packaging
# (tools/package-release.sh; Alpine's busybox has neither).
RUN apk add --no-cache build-base git make curl ca-certificates unzip perl bash zip \
